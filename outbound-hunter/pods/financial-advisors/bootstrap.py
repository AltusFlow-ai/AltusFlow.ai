"""
pods/financial-advisors/bootstrap.py
Pre-flight validation for the Financial Advisors pod.

Run on pod startup. Validates environment, pod files, DB, niche module, and HubSpot.
If any check fails, pod does not start and the circuit breaker is tripped.

Usage:
  python pods/financial-advisors/bootstrap.py          # run standalone
...
def validate() -> dict:
    """
    Run all pre-flight checks for the financial-advisors pod.
    ...
    # ── 3. Database (also initialises tables if first run) ───────────────────
    try:
